...
func (s *POSService) CreatePOSTransaction(merchantID *string, contactID, warehouseID string, items []POSSalesItemModel, description string) (*POSModel, error) {
	...
	err := s.ctx.DB.Transaction(func(tx *gorm.DB) error {
		...
		for _, item := range items {
			if err := invSrv.StockMovementService.AddMovement(now, *item.ProductID, warehouseID, merchantID, nil, -item.Quantity, stockmovement.MovementTypeOut, pos.ID, description); err != nil {
				return err
			}
		}
