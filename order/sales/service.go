...
func (s *SalesService) UpdateStock(salesID, warehouseID string, description string) error {
	...
	err := s.ctx.DB.Transaction(func(tx *gorm.DB) error {
		for _, v := range sales.Items {
			...
			_, err := invSrv.StockMovementService.AddMovement(sales.SalesDate, *v.ProductID, *v.WarehouseID, v.VariantID, nil, nil, -v.Quantity, models.MovementTypeIn, sales.ID, description)
			if err != nil {
				tx.Rollback()
				return err
			}
