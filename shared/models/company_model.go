...
type CompanyModel struct {
	...
	CashflowGroupSetting     *CashflowGroupSetting `gorm:"-" json:"cashflow_group_setting,omitempty"`
	CashflowGroupSettingData *string               `json:"cashflow_group_setting_data,omitempty" gorm:"type:JSON"`
}
...
func (c *CompanyModel) AfterFind(tx *gorm.DB) (err error) {
	if c.CashflowGroupSettingData != nil {
		if err = json.Unmarshal([]byte(*c.CashflowGroupSettingData), &c.CashflowGroupSetting); err != nil {
			return err
		}
	}
	if c.CashflowGroupSettingData == nil {
		defaultSetting := DefaultCasflowGroupSetting()
		if c.IsCooperation {
			defaultSetting = CooperationCasflowGroupSetting()
		}
		c.CashflowGroupSetting = &defaultSetting
		b, _ := json.Marshal(defaultSetting)
		settingStr := string(b)
		c.CashflowGroupSettingData = &settingStr
		tx.Save(c)
	}
	return
}
